...
import com.cici.oauth.config.ClientIdType;
...
public class CustomAuthenticationProvider implements AuthenticationProvider {
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        ...
        log.info("clientDetail:" + clientDetails.toString());
        //检查客户端与用户信息是否吻合
    ...
    public void clientChecks(UserDetails userDetails, ClientDetails clientDetails) {
        ...
        if (ClientIdType.APP_CLIENT.getId().equals(clientDetails.getClientId())) {
            if (!isHasClientIdAuthorities(userDetails, clientDetails)) {
                log.info("------------------------用户信息：" + userDetails.toString());
                log.info("------------------------客户端信息：" + clientDetails.toString());
